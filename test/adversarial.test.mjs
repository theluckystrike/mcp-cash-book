...
test.skip("an invoice paid twice, once by payment row and once by deposit, posts the cash ONCE and says the books disagree", async (t) => {
...
test.skip("a bank row that could be either of two expenses is matched to neither", async (t) => {
  const { box, c } = open(t, { key: proKey() });
...
test.skip("a deposit applied to an invoice that does not exist is reported and never posted", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  seed.invoices(box.dataHome, []);
  seed.deposits(box.dataHome, [deposit({
    id: "DEP-2026-0001", amount_minor: 100000, received_date: "2026-06-01",
...
test.skip("month_close, the CSV and the report are Pro, and a key for another product unlocks nothing", async (t) => {
  const { box, c } = open(t);
  workedMonth(box.dataHome);
  await c.init();
...
test.skip("a credit note stored with a positive total is posted as it stands and flagged, never silently flipped", async (t) => {
  const { box, c } = open(t, { key: proKey() });
...
test.skip("a period a closed month stands on is refused, and the close is named", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  workedMonth(box.dataHome);
  await c.init();
