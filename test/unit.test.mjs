...
test.skip("month_close lists what is unposted or inconsistent and records the trial balance snapshot", async (t) => {
...
test.skip("closing a month again after a sibling store moved reports the drift instead of hiding it", async (t) => {
...
test.skip("the CSV is one row per leg, quoted, with the source columns", async (t) => {
  const { c } = await worked(t, { key: proKey() });
...
test.skip("ledger_report states the movement and the balance of every account, and says which side it reads as", async (t) => {
  const { c } = await worked(t, { key: proKey() });
