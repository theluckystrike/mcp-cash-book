...
test.skip("twenty periods built from two processes leave twenty rows and no lost write", async (t) => {
...
test.skip("two processes closing the same month leave one close row", async (t) => {
  const { box, a, b } = pair(t, { key: proKey() });
  await Promise.all([a.init(), b.init()]);
